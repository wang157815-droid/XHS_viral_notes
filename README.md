...
**Tab 2: RAG 文档**
1. **上传文档**：支持 PDF、Word、Markdown、TXT 等文档格式
2. **自动解析**：上传后自动分块并向量化
3. **文档管理**：查看、搜索和删除已上传文档
4. **知识库检索**：对话问答和分析流程可复用 RAG 文档片段
...
**核心优势**：
- ✅ **维护简单**：不再维护领域配置和关键词规则
- ✅ **语义检索**：通过向量相似度召回相关文档片段
- ✅ **文档驱动**：上传案例、规范、经验总结即可复用
- ✅ **接口统一**：对话问答与分析流程共用 RAG 检索能力

### 文档管理

1. 访问知识库页面
2. 上传 PDF、Word、Markdown 或 TXT 文档
3. 系统自动解析、分块并向量化
4. 可查看文档、检索分块或删除文档

### 最佳实践

1. **文档质量**：上传结构清晰、主题明确的资料
2. **命名规范**：文件名体现品牌、品类或用途
3. **定期更新**：淘汰过期资料，保持知识库内容新鲜
4. **检索验证**：上传后使用关键词或问题确认召回效果

### 故障排查

**Q: 检索无结果？**
- A: 检查文档是否已成功向量化，并确认 Embedding 配置可用

**Q: 回答引用不准确？**
- A: 优先上传更聚焦的文档，避免单个文档混杂过多主题
...
视频分析可复用已上传的 RAG 文档片段，为产品植入、内容结构、CTA 等分析提供参考资料。
...
支持通过小红书 APP 扫码自动获取 Cookie，免去手动复制粘贴的繁琐步骤。

**核心特性**：
- ✅ **双模式支持**：桌面模式弹出浏览器窗口，服务器模式显示实时截图
...
2. 点击侧边栏「Cookie 配置」中的「扫码登录」按钮
3. 根据运行环境：
   - **桌面模式**：浏览器窗口自动弹出，在窗口中完成扫码和验证
   - **服务器模式**：页面显示小红书登录页截图，用手机扫描截图中的二维码
...
│                    前端 (index.html)                             │
│  ┌──────────────┐    ┌──────────────┐    ┌──────────────────┐   │
│  │ 扫码登录按钮  │ →  │ 截图/验证码   │ →  │ 状态轮询(1.5s)    │   │
│  └──────────────┘    └──────────────┘    └──────────────────┘   │
└─────────────────────────────┬───────────────────────────────────┘
                              │ REST API
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│                    后端 FastAPI (viral_app.py)                   │
│  POST /api/qrcode/session      → 创建扫码会话                    │
│  GET  /api/qrcode/{id}/status  → 获取状态+截图                   │
│  POST /api/qrcode/{id}/sms     → 提交短信验证码                  │
│  DELETE /api/qrcode/{id}       → 取消会话                        │
└─────────────────────────────┬───────────────────────────────────┘
                              │
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│              QRCodeLoginService (核心服务)                        │
...
```
INITIALIZING → WAITING_SCAN → NEED_SMS_CODE → SCANNED → CONFIRMED → SUCCESS
                    ↓              ↓              ↓
                 EXPIRED        (输入验证码)    ERROR
```
...
| `waiting_scan` | 等待用户扫码 |
| `need_sms_code` | 需要输入短信验证码 |
| `scanned` | 已扫码，正在验证 Cookie |
| `confirmed` | Cookie 验证通过 |
...
```
viral_agent/services/auth/           # 扫码登录模块
├── __init__.py                      # 模块导出
├── qrcode_session.py               # 会话模型 + 状态枚举
└── qrcode_login_service.py         # Playwright 控制 + Cookie 提取
```
...
| 登录超时 | 二维码有效期 | 180秒 |
| 截图间隔 | 服务器模式截图刷新 | 3秒 |
| 截图格式 | JPEG 压缩 | quality=70 |
| 验证重试 | Cookie 验证失败后重试 | 5秒 |

### 常见问题

**Q: 云服务器上无法使用？**
A: 系统会自动检测环境。云服务器使用 headless 浏览器 + 截图模式，无需图形界面。

**Q: 出现滑块验证怎么办？**
A: 滑块验证需要在网页中交互，截图模式无法完成。请使用手动复制 Cookie 方式。

**Q: 短信验证码输入后没反应？**
A: 系统会自动将验证码填入网页并提交，然后继续监听 Cookie 变化。请等待几秒钟。
...
│              知识库系统（RAG 文档）                         │
├─────────────────────────────────────────────────────────┤
│ • 文档上传（Word/PDF/MD/TXT）                              │
│ • 自动解析向量化                                           │
│ • ChromaDB / pgvector 存储                                 │
│ • 智能语义检索                                             │
├─────────────────────────────────────────────────────────┤
│          统一检索接口（文档知识）                           │
└─────────────────────────────────────────────────────────┘
